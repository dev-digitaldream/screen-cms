...
router.get('/', requireAuth, (req, res) => {
  ...
  res.json(devices.map(d => ({
    ...
    isOnline: d.last_seen && (Date.now() - new Date(d.last_seen).getTime()) < 90_000,
  })))
})
...
router.post('/:id/heartbeat', (req, res) => {
  ...
  if (config.forceReload || config.clearCache) {
    const reset = { ...config, forceReload: false, clearCache: false }
