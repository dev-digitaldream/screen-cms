...
function onlineStatus(lastSeen) {
  ...
  if (diff < 20 * 60_000) return 'recent'
  return 'offline'
}
...
function DeviceCard({ device, screens, onRefresh, onDelete }) {
  ...
  async function handleSleep() {
    ...
    await fetch(`/api/v1/devices/${device.id}`, {
      ...
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ config: { ...cfg, sleep: true } }),
    })
    onRefresh()
  }
  ...
  async function handleWake() {
    ...
    await fetch(`/api/v1/devices/${device.id}`, {
      ...
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ config: { ...cfg, wake: true } }),
    })
    onRefresh()
  }
  ...
  async function handleReboot() {
    ...
    await fetch(`/api/v1/devices/${device.id}`, {
      ...
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ config: { ...cfg, reboot: true } }),
    })
    onRefresh()
  }
      ...
      <div style={{
        ...
        <div style={{ padding: '14px 16px' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            ...
            <button onClick={handleSleep} title="Mettre en veille maintenant" style={btnSecondarySmallStyle}>
              <Moon size={12} style={{ marginRight: 4 }} />
              Veille
            </button>
            <button onClick={handleWake} title="Réveiller maintenant" style={btnSecondarySmallStyle}>
              <Sun size={12} style={{ marginRight: 4 }} />
              Réveil
            </button>
            <button onClick={handleReboot} title="Redémarrer l'appareil" style={{ ...btnSecondarySmallStyle, color: '#f59e0b' }}>
...
export default function DevicesPanel({ onBack }) {
  ...
  const onlineCount = devices.filter(d => ['online', 'recent'].includes(onlineStatus(d.last_seen))).length
          ...
          <div style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: 'var(--ink)' }}>
                APK disponible — Screen Editor Player v{apkInfo.versionName || '1.0.0'}
              </p>
